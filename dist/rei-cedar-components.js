...
riot.tag2('cdr-event-date', '<section class="event"> <div class="event__date text-uppercase"> <div class="h2 event__title">{opts.item.dateDayOfWeek}</div> <div class="text-muted">{opts.item.dateMonthAndDayOfMonth}</div> </div> <div class="event__content"> <h4 class="event__title">{opts.item.title}</h4> <div class="event__location text-muted">{opts.item.location}</div> </div> </section>', '.event{display:flex;flex-flow:row nowrap;justify-content:flex-start}.event .event__date{padding-right:1rem;white-space:nowrap}.event .event__content{border-left:1px solid #ccc;padding-left:1rem}.event .event__title{font-family:inherit}.event .event__title.h2{margin-bottom:0;line-height:1}.event .event__location{line-height:1}', '', function(opts) {
});