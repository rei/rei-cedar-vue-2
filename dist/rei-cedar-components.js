...
riot.tag2('cdr-event-date', '<section class="event"> <div class="event__date text-uppercase"> <div class="h2 event__title">{opts.item.dateDayOfWeek}</div> <div class="text-muted">{opts.item.dateMonthAndDayOfMonth}</div> </div> <div class="event__content"> <h4 class="event__title">{opts.item.title}</h4> <p class="event__location">{opts.item.location}</p> <div if="{opts.item.spots > 0 || opts.item.spots == 0}" class="event__availability"> <div if="{opts.item.spots > 0}" class="text-muted">{opts.item.spots} spots available</div> <div class="event__waitList" if="{opts.item.spots == 0}"><span class="text-off-price">Event is full.</span> <a href="{opts.item.waitListUrl}" class="link_navigation text-nowrap">Join wait list</a></div> </div> </div> </section>', 'cdr-event-date .event,[riot-tag="cdr-event-date"] .event,[data-is="cdr-event-date"] .event{display:flex;flex-flow:row nowrap;justify-content:flex-start} cdr-event-date .event__location:last-child,[riot-tag="cdr-event-date"] .event__location:last-child,[data-is="cdr-event-date"] .event__location:last-child{margin-bottom:0} cdr-event-date .event__date,[riot-tag="cdr-event-date"] .event__date,[data-is="cdr-event-date"] .event__date{padding-right:1rem;white-space:nowrap}@media screen and (max-width:768px){ cdr-event-date .event__date,[riot-tag="cdr-event-date"] .event__date,[data-is="cdr-event-date"] .event__date{min-width:7.5rem}} cdr-event-date .event__content,[riot-tag="cdr-event-date"] .event__content,[data-is="cdr-event-date"] .event__content{border-left:1px solid #ccc;padding-left:1rem} cdr-event-date .event__title,[riot-tag="cdr-event-date"] .event__title,[data-is="cdr-event-date"] .event__title{font-family:inherit} cdr-event-date .event__title.h2,[riot-tag="cdr-event-date"] .event__title.h2,[data-is="cdr-event-date"] .event__title.h2{margin-bottom:0;line-height:1}@media screen and (min-width:1200px){ cdr-event-date .event__title.h2,[riot-tag="cdr-event-date"] .event__title.h2,[data-is="cdr-event-date"] .event__title.h2{font-size:2.843rem}} cdr-event-date .event__waitList a,[riot-tag="cdr-event-date"] .event__waitList a,[data-is="cdr-event-date"] .event__waitList a{text-decoration:underline}@media screen and (min-width:768px){ cdr-event-date .event__waitList>*,[riot-tag="cdr-event-date"] .event__waitList>*,[data-is="cdr-event-date"] .event__waitList>*{display:block}}', '', function(opts) {
});
...
riot.tag2('cdr-modal-video', '<div> <button type="button" class="btn btn-play" onclick="{showModal}"> <span class="icon icon-rei-play-small" aria-hidden="true"></span><span>{opts.buttonText}</span> </button> <aside if="{opened}" class="cdr-modal" tabindex="-1" role="dialog" aria-hidden="true" aria-labelledby="dialog1Title"> <span id="dialog1Title" class="sr-only">{\'Begin dialog for\' + opts.videoTitle}</span> <div class="cdr-modal__content"> <div class="text-right"> <button onclick="{hideModal}" class="close icon icon-rei-close img-circle" aria-label="Close"></button> </div> <section class="cdr-modal__content--inner"> <div class="embed-responsive embed-responsive-16by9"> <iframe title="{opts.videoTitle}" width="900" height="506" riot-src="https://www.youtube.com/embed/{opts.videoId}" frameborder="0" allowfullscreen></iframe> </div> </section> </div> <span class="sr-only">End dialog</span> </aside> </div>', 'cdr-modal-video .icon.icon-rei-close.img-circle,[riot-tag="cdr-modal-video"] .icon.icon-rei-close.img-circle,[data-is="cdr-modal-video"] .icon.icon-rei-close.img-circle{background:rgba(0,0,0,0.5);color:#fff;padding:2rem;margin:1.6rem;float:none}@media screen and (max-width:768px){ cdr-modal-video .icon.icon-rei-close.img-circle,[riot-tag="cdr-modal-video"] .icon.icon-rei-close.img-circle,[data-is="cdr-modal-video"] .icon.icon-rei-close.img-circle{position:absolute;top:0;right:0;bottom:auto;left:auto;z-index:1}}', '', function(opts) {
